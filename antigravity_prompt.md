...
## 📁 Cấu Trúc File Dự Án (Kiến Trúc Phân Tầng Mới)

```
f:\train AI đồ án\
│
├── ⚙️ configs/                          ← Cấu hình dataset và hyperparameters
│   ├── pothole_dataset.yaml           ← Dataset config cho tập Pothole (Roboflow)
│   └── crack_8classes.yaml            ← Dataset config cho 8 classes RoadGuard
│
├── 🧠 src/                              ← Mã nguồn phân tầng theo 5 tầng nghiệp vụ
│   ├── ingestion/                     ← [TẦNG 1] video_extractor.py, srt_parser.py
│   ├── inference/                     ← [TẦNG 2 & 3] tiling_engine.py, predictor.py
│   ├── measurement/                   ← [TẦNG 4] metric_estimator.py, spatial_tracker.py
│   ├── contracts/                     ← [TẦNG 5] aidetection_schema.py, mock_ai_adapter.py
│   └── training/                      ← train_pothole.py, train_crack.py
│
├── 🚀 pipeline.py                       ← MASTER RUNNER: Video + SRT -> JSON Report
│
├── 📓 notebooks/                        ← Cloud training notebooks (Colab/Kaggle T4)
│   └── train_pothole_colab_kaggle.ipynb
│
├── ⚖️ weights/                          ← Quản lý trọng số mô hình
│   ├── base/best.pt                   ← YOLOv11n pretrained base weights
│   └── trained/                       ← Model sau khi huấn luyện (pothole_best.pt)
│
├── 📦 data/                             ← Dữ liệu hình ảnh
│   └── pothole/                       ← 3,940 ảnh & nhãn (train, valid, test)
│
├── 📚 docs/                             ← Toàn bộ tài liệu nghiệp vụ & tiến độ
│   ├── tiến_độ.md                     ← Bảng theo dõi tiến độ & Agent Rule
│   ├── ke_hoach_thuc_thi_AI.md        ← Master Technical Blueprint (6 Epics)
│   ├── AI_task.md                     ← Đặc tả chi tiết nhiệm vụ AI
│   └── specs/                         ← 5 tài liệu nghiệp vụ RoadGuard gốc
│
├── 🧪 tests/                            ← Kiểm thử tự động (test_pipeline.py)
├── 📄 requirements.txt                  ← Python dependencies
└── 📄 antigravity_prompt.md             ← File này (Master Project Document)
```
